...
# ══════════════════════════════════════════════════════════════════════════════
# FORECAST LOADER
# Handles your exact format: "March Forecast", "Feb Forecast", "Jan forecast"
# i.e.  <MonthName> Forecast  OR  Forecast <MonthName>  (case-insensitive)
# ══════════════════════════════════════════════════════════════════════════════
...
def col_to_month_num(col_name: str):
    """
    Parse a column name like 'March Forecast', 'Feb Forecast',
    'Jan forecast', 'Forecast March', etc.
    Returns month_int (1-12) or None.
    """
    ...
    for i, abbr in enumerate(MONTH_ABBR):
        # Match whole word to avoid 'dec' matching 'december' twice etc.
        import re
...
def load_forecast_by_month():
    """
    Returns dict: { month_int -> pd.DataFrame(columns=['_key','Forecast']) }
    Reads the YogaBar wide-format Forecast sheet where columns are named
    'March Forecast', 'Feb Forecast', 'Jan forecast', 'Dec Forecast', 'Nov Forecast'
    """
    ...
    if not ic_col:
        ic_col = next((c for c in df_fc.columns if "sku" in c.lower()), None)
    if not ic_col:
        return {}, [], "no_item_col"

    df_fc[ic_col] = df_fc[ic_col].astype(str).str.strip().str.upper()

    # Detect forecast columns — any column whose name contains a month name AND "forecast"
    fc_cols = {}   # month_int -> col_name
    detected_cols = []
    ...
    if not fc_cols:
        return {}, [], "no_forecast_cols"

    # Build per-month forecast dict
    result = {}
...
def load_consumption():
    ...
    mat_col  = next((c for c in df_con.columns
                     if "material" in c.lower() and ("code" in c.lower() or "sku" in c.lower())), None)
    if not mat_col:
...
def build_data():
    ...
    df_con, mat_col, con_col, name_col, cat_col = load_consumption()

    if df_con.empty or not mat_col or not con_col:
        return pd.DataFrame(), fc_fmt, detected_cols

    frames = []
    for minfo in MONTHS:
        if "_month" not in df_con.columns:
            continue

        # Consumption for this month
        mdf = df_con[
            (df_con["_month"] == minfo["month"]) &
            (df_con["_year"]  == minfo["year"])
        ].copy()
        if mdf.empty:
            continue
        ...
        # ── Match THIS month's forecast by month number ───────────────────
        ...
        agg["Var %"]     = agg.apply(
            ...
            if r["Expected"] > 0 else 0, axis=1)
        agg["Status"]    = agg["Variance"].apply(
            lambda v: "Over" if v > 0 else ("Under" if v < 0 else "On Track"))
        agg["FC Mapped"] = "✓" if minfo["month"] in fc_dict else "⚠ missing"

        # Only rows with actual consumption
...
# ── Info box showing what was detected ────────────────────────────────────────
if detected_cols:
    cols_str = " · ".join(detected_cols)
    st.markdown(
        f'<div class="info-box">'
        f'✅ Forecast columns detected: <b>{cols_str}</b><br>'
        f'Each month\'s consumption is now matched to its own forecast column.'
        f'</div>',
        unsafe_allow_html=True
    )
elif fc_fmt == "no_forecast_cols":
    st.warning("⚠️ No forecast columns found. Make sure columns contain both a month name and the word 'Forecast' — e.g. 'March Forecast', 'Feb Forecast'.")
elif fc_fmt == "empty":
    st.error("⚠️ Forecast sheet is empty.")

if df_full.empty:
    st.error("⚠️ No consumption data found for the last 5 months.")
    st.stop()
...
# ══════════════════════════════════════════════════════════════════════════════
st.markdown('<div class="sec">📅 Monthly Summary — Each Month Uses Its Own Forecast Column</div>', unsafe_allow_html=True)
...
for i, minfo in enumerate(MONTHS):
    ...
    # Show which forecast column is mapped
    fc_col_used = next(
        (c for c in detected_cols if str(minfo["month"]) in c.split("→")[1]),
        None
    ) if detected_cols else None
    fc_note = fc_col_used.split("→")[0].strip() if fc_col_used else ("⚠ no forecast" if not exp else "")
    ...
    <div class="kpi-box {MONTH_CSS[i]}">
      ...
      <div style="margin-top:5px;font-size:9px;color:#334155;font-family:'JetBrains Mono',monospace;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{fc_note}</div>
    </div>"""
html += '</div>'
...
with pd.ExcelWriter(buf, engine="openpyxl") as w:
    ...
    try:
        pv = df.pivot_table(
            index="Material SKU", columns="Month", values="Actual", aggfunc="sum"
        ).fillna(0)
        pv.to_excel(w, sheet_name="SKU Pivot")
    except Exception:
        pass
st.download_button(
    "⬇  Export to Excel", buf.getvalue(), "Consumption_5Month.xlsx",
