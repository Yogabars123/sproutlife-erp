...
.live-pill {
    ...
    background:#071a0f; border:1px solid #166534; border-radius:20px;
    padding:5px 11px; font-size:10px; font-weight:700; color:#22c55e;
    letter-spacing:1px; font-family:'JetBrains Mono',monospace; flex-shrink:0;
}
.live-dot {
    width:6px; height:6px; min-width:6px; background:#22c55e; border-radius:50%;
    animation:blink 1.8s ease-in-out infinite;
}
@keyframes blink {
    0%,100%{opacity:1;box-shadow:0 0 5px #22c55e;} 50%{opacity:0.2;box-shadow:none;}
}
...
.kpi-lbl { font-size:10px; font-weight:700; color:#a78bfa; text-transform:uppercase; letter-spacing:1.3px; margin-bottom:6px; }
.kpi-num {
    font-size:36px; font-weight:800;
    background:linear-gradient(135deg,#e879f9,#818cf8,#38bdf8);
    -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text;
    letter-spacing:-1.5px; line-height:1; font-family:'JetBrains Mono',monospace;
}
...
[data-testid="stTextInput"] input {
    ...
    font-size:13px !important; padding:9px 12px !important; border:none !important; box-shadow:none !important;
}
...
.stButton > button {
    width:100% !important; background:#0d1117 !important; border:1.5px solid #161d2e !important;
...
.stButton > button:hover { border-color:#7c3aed !important; color:#a78bfa !important; background:#160b2e !important; }

.stDownloadButton > button {
    width:100% !important; background:linear-gradient(135deg,#0f172a,#1e1b4b) !important;
    border:1.5px solid #4338ca !important; border-radius:9px !important; color:#a5b4fc !important;
    font-size:13px !important; font-weight:700 !important; padding:10px !important;
...
.sec-div {
    font-size:10px; font-weight:700; color:#1e2d45; text-transform:uppercase; letter-spacing:1.2px;
    padding:12px 0 8px 0; display:flex; align-items:center; gap:7px;
}
.sec-div::after { content:''; flex:1; height:1px; background:#161d2e; }
div[data-testid="stDataFrame"] { border-radius:12px !important; overflow:hidden !important; border:1px solid #161d2e !important; }
[data-testid="stAlert"] { background:#1a1200 !important; border:1px solid #78350f !important; border-radius:10px !important; color:#fbbf24 !important; }
.app-footer { margin-top:2rem; padding-top:12px; border-top:1px solid #161d2e; text-align:center; font-size:10px; font-weight:600; color:#1e2d45; letter-spacing:1.5px; font-family:'JetBrains Mono',monospace; }
</style>
...
if is_filtered:
    ...
    ctx   = f'"{search}"' if search else (sel_wh[:28] if sel_wh != "All Warehouses" else (sel_cat[:28] if sel_cat != "All Categories" else sel_st))
    st.markdown(f"""
...
st.markdown(f'<div class="tbl-hdr"><span class="tbl-lbl">📋 All columns</span><span class="tbl-badge">{len(df):,} rows</span></div>', unsafe_allow_html=True)
...
if df.empty:
    ...
    priority = ["Item Name","Item SKU","Category","Warehouse","UoM",
                ...
                "Batch No","MFG Date","Expiry Date","Current Aging (Days)","Inventory Date",
