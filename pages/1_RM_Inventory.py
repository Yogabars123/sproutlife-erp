...
def load_rm():
    ...
    for c in ["Qty Available", "Qty Inward", "Qty (Issue / Hold)",
...
with col_w:
    ...
    selected_wh = st.selectbox("w", wh_opts, label_visibility="collapsed")
with col_c:
    cat_opts = (["All Categories"] +
                sorted(df_raw["Category"].dropna().astype(str).unique().tolist())
                if "Category" in df_raw.columns else ["All Categories"])
    selected_cat = st.selectbox("c", cat_opts, label_visibility="collapsed")
with col_st:
    stock_filter = st.selectbox("st",
                                ["All Stock", "Available Only", "Zero / Neg"],
                                label_visibility="collapsed")
...
if is_filtered:
    ...
    <div class="result-banner">
        <div>
            <div class="rb-lbl">Filtered · {ctx}</div>
            <div class="rb-val">{f_qty:,.0f}</div>
        </div>
        <div class="rb-right">
            <div class="rb-cnt">{len(df):,}</div>
            <div class="rb-sub">records</div>
        </div>
    </div>
    """, unsafe_allow_html=True)
...
if df.empty:
    ...
    priority = [
        ...
        "Current Aging (Days)", "Inventory Date",
