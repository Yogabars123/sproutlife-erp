...
with tab2:
    ...
    # ── Extra CSS (dark-theme compatible, matches existing dashboard) ────────────
    st.markdown("""
    <style>
    ...
    .cfa-hdr {
        ...
        padding:9px 16px;margin:14px 0 10px;border-radius:0 6px 6px 0;
    }
    .cfa-hdr-title {
        font-size:11px;font-weight:700;color:#93c5fd;
        letter-spacing:1px;text-transform:uppercase;font-family:'IBM Plex Mono',monospace;
    }
    .cfa-hdr-badge {
        background:#1e3a5f;border:1px solid #3b82f6;color:#93c5fd;
        font-size:10px;font-weight:700;padding:2px 10px;border-radius:3px;
    ...
    .mode-badge-with {
        ...
        padding:3px 10px;background:#0a1e3d;color:#60a5fa;
        border:1px solid #1e3a5f;border-radius:4px;font-family:'IBM Plex Mono',monospace;
    }
    .mode-badge-without {
        display:inline-block;font-size:10px;font-weight:700;
        padding:3px 10px;background:#2d1800;color:#fb923c;
        border:1px solid #78350f;border-radius:4px;font-family:'IBM Plex Mono',monospace;
    }

    /* Formula strip */
    .cfa-formula {
        background:#080e1a;border:1px solid #1e2d45;border-left:3px solid #3b82f6;
        border-radius:0 8px 8px 0;padding:8px 16px;margin-bottom:12px;
        font-size:11px;font-family:'IBM Plex Mono',monospace;color:#475569;
        display:flex;gap:8px;align-items:center;flex-wrap:wrap;
    }
    .cfa-formula .ft { background:#1e3a5f;color:#93c5fd;padding:1px 8px;border-radius:3px;font-weight:700;font-size:10px; }
    .cfa-formula .fo { color:#3b82f6;font-weight:700;font-size:14px; }
    .cfa-formula .fn { color:#334155;font-size:10px;font-style:italic; }

    /* KPI grid */
    .cfa-kpi-grid { display:grid;grid-template-columns:repeat(5,1fr);gap:10px;margin-bottom:16px; }
    .cfa-kpi {
        border-radius:12px;padding:14px 16px;border:1px solid;position:relative;overflow:hidden;
    }
    .cfa-kpi::before {
        content:'';position:absolute;top:0;left:0;right:0;height:3px;border-radius:12px 12px 0 0;
    }
    .cfa-kpi.blue  { background:linear-gradient(135deg,#060e1a,#0a1a2e);border-color:#1e3a5f; }
    .cfa-kpi.blue::before  { background:linear-gradient(90deg,#3b82f6,#60a5fa); }
    .cfa-kpi.slate { background:linear-gradient(135deg,#0a0e16,#0f1520);border-color:#1e2535; }
    .cfa-kpi.slate::before { background:linear-gradient(90deg,#475569,#64748b); }
    .cfa-kpi.amber { background:linear-gradient(135deg,#1a1000,#2a1800);border-color:#78350f; }
    .cfa-kpi.amber::before { background:linear-gradient(90deg,#f59e0b,#fbbf24); }
    .cfa-kpi.green { background:linear-gradient(135deg,#061a0a,#0a2e12);border-color:#14532d; }
    .cfa-kpi.green::before { background:linear-gradient(90deg,#22c55e,#4ade80); }
    .cfa-kpi.red   { background:linear-gradient(135deg,#1a0000,#2a0808);border-color:#7f1d1d; }
    .cfa-kpi.red::before   { background:linear-gradient(90deg,#ef4444,#f87171); }
    .cfa-kpi.orange{ background:linear-gradient(135deg,#1a0800,#2a1000);border-color:#9a3412; }
    .cfa-kpi.orange::before{ background:linear-gradient(90deg,#f97316,#fb923c); }

    .cfa-kpi-lbl { font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:1.2px;margin-bottom:5px; }
    .cfa-kpi-val { font-size:22px;font-weight:800;line-height:1;font-family:'IBM Plex Mono',monospace;letter-spacing:-0.5px; }
    .cfa-kpi-sub { font-size:10px;margin-top:4px; }
    .cfa-kpi.blue  .cfa-kpi-lbl{color:#60a5fa;}.cfa-kpi.blue  .cfa-kpi-val{color:#bfdbfe;}.cfa-kpi.blue  .cfa-kpi-sub{color:#2563eb;}
    .cfa-kpi.slate .cfa-kpi-lbl{color:#94a3b8;}.cfa-kpi.slate .cfa-kpi-val{color:#cbd5e1;}.cfa-kpi.slate .cfa-kpi-sub{color:#475569;}
    .cfa-kpi.amber .cfa-kpi-lbl{color:#fbbf24;}.cfa-kpi.amber .cfa-kpi-val{color:#fde68a;}.cfa-kpi.amber .cfa-kpi-sub{color:#d97706;}
    .cfa-kpi.green .cfa-kpi-lbl{color:#4ade80;}.cfa-kpi.green .cfa-kpi-val{color:#bbf7d0;}.cfa-kpi.green .cfa-kpi-sub{color:#16a34a;}
    .cfa-kpi.red   .cfa-kpi-lbl{color:#f87171;}.cfa-kpi.red   .cfa-kpi-val{color:#fecaca;}.cfa-kpi.red   .cfa-kpi-sub{color:#dc2626;}
    .cfa-kpi.orange.cfa-kpi-lbl{color:#fb923c;}.cfa-kpi.orange .cfa-kpi-val{color:#fed7aa;}.cfa-kpi.orange .cfa-kpi-sub{color:#ea580c;}

    /* Fill rate panel */
    .fill-panel { background:#0d1117;border:1px solid #1e2535;border-radius:12px;padding:14px; }
    .fill-panel-title { font-size:10px;font-weight:700;color:#475569;text-transform:uppercase;letter-spacing:1px;border-bottom:1px solid #1e2535;padding-bottom:7px;margin-bottom:10px; }
    .fill-row { display:grid;grid-template-columns:160px 1fr 62px 54px;align-items:center;gap:10px;padding:6px 0;border-bottom:1px solid #111827; }
    .fill-row:last-child { border-bottom:none; }
    .fill-cfa-name { font-size:11px;font-weight:600;color:#cbd5e1;white-space:nowrap;overflow:hidden;text-overflow:ellipsis; }
    .fill-bar-wrap { background:#1e2535;border-radius:3px;height:8px; }
    .fill-bar-inner { height:8px;border-radius:3px; }
    .fill-pct { font-size:11px;font-weight:700;font-family:'IBM Plex Mono',monospace;text-align:right; }
    .fill-short { font-size:10px;font-weight:700;text-align:right;font-family:'IBM Plex Mono',monospace; }

    /* Status chips */
    .chip-ok   { display:inline-block;font-size:9px;font-weight:700;padding:1px 7px;background:#061a0a;color:#4ade80;border:1px solid #14532d;border-radius:3px; }
    .chip-warn { display:inline-block;font-size:9px;font-weight:700;padding:1px 7px;background:#2d1800;color:#fbbf24;border:1px solid #78350f;border-radius:3px; }
    .chip-crit { display:inline-block;font-size:9px;font-weight:700;padding:1px 7px;background:#2d0a0a;color:#f87171;border:1px solid #7f1d1d;border-radius:3px; }

    /* Expiry heatmap */
    .hm-wrap { background:#0d1117;border:1px solid #1e2535;border-radius:12px;overflow:hidden; }
    .hm-head { display:grid;grid-template-columns:150px repeat(5,1fr);background:#0a1628; }
    .hm-hcell { padding:8px 10px;font-size:9px;font-weight:700;font-family:'IBM Plex Mono',monospace;color:#64748b;text-transform:uppercase;letter-spacing:0.5px;text-align:center;border-right:1px solid #1e2535; }
    .hm-hcell:first-child { text-align:left; }
    .hm-row { display:grid;grid-template-columns:150px repeat(5,1fr);border-bottom:1px solid #111827; }
    .hm-row:last-child { border-bottom:none; }
    .hm-row:hover { background:#0a0f1a; }
    .hm-cell { padding:7px 10px;font-size:11px;font-family:'IBM Plex Mono',monospace;text-align:center;border-right:1px solid #111827; }
    .hm-cell:first-child { text-align:left;font-weight:600;color:#cbd5e1;font-family:'JetBrains Mono',monospace;font-size:11px; }
    .hm-total { display:grid;grid-template-columns:150px repeat(5,1fr);background:#080e1a;border-top:1px solid #1e2535; }
    .hm-tcell { padding:8px 10px;font-size:11px;font-weight:700;font-family:'IBM Plex Mono',monospace;text-align:center;border-right:1px solid #1e2535;color:#94a3b8; }
    .hm-tcell:first-child { text-align:left;color:#64748b; }

    /* Batch detail panel */
    .batch-panel {
        background:#060d18;border:1.5px solid #3b82f6;border-radius:12px;
        padding:14px 18px;margin-bottom:12px;
    }
    .batch-panel-title { font-size:10px;font-weight:700;color:#60a5fa;text-transform:uppercase;letter-spacing:1px;margin-bottom:10px; }
    .batch-row { display:flex;align-items:center;gap:14px;padding:5px 0;border-bottom:1px solid #0f1a2e; }
    .batch-row:last-child { border-bottom:none; }

    /* Hint box */
    .hint-box {
        background:#060d18;border:1px dashed #1e2535;border-radius:10px;
        padding:12px 18px;margin-bottom:12px;text-align:center;
        color:#334155;font-size:11px;font-family:'IBM Plex Mono',monospace;
    }
    </style>
    """, unsafe_allow_html=True)
    ...
    _mc1, _mc2, _mc3 = st.columns([1.2, 1.5, 1.5])
    with _mc1:
        st.markdown('<div class="mode-strip-label">📐 Availability Mode</div>', unsafe_allow_html=True)
    with _mc2:
    ...
    # ── Formula strip ────────────────────────────────────────────────────────────
    _stn_part = '<span class="fo">+</span> <span class="ft">STN In-Transit</span>' if include_stn \
                else '<span class="fn">(STN excluded from calc)</span>'
    _mode_badge_cls = "mode-badge-with" if include_stn else "mode-badge-without"
    st.markdown(f"""
    <div class="cfa-formula">
        <span class="ft">DIFF</span>
        <span class="fo">=</span>
        <span class="ft">CFA FG Stock</span>
        {_stn_part}
        <span class="fo">−</span>
        <span class="ft">Open PO Qty</span>
        <span class="fn">· Excl. Cancelled &amp; Closed SOs</span>
        &nbsp;|&nbsp;
        <span class="{_mode_badge_cls}">Mode: {stn_mode}</span>
    </div>
    """, unsafe_allow_html=True)

    # ── Derived KPI values ───────────────────────────────────────────────────────
    if include_stn:
        _total_avail_t2 = cfa_fg + stn_cfa_qty
        _avail_label    = "FG + STN In-Transit"
        _stn_tile_cls   = "blue"
        _stn_sub        = "Included in calculation"
    else:
        _total_avail_t2 = cfa_fg
        _avail_label    = "FG Stock Only"
        _stn_tile_cls   = "slate"
        _stn_sub        = "Excluded from calculation"

    _diff_t2  = _total_avail_t2 - open_so_qty
    _diff_cls = "green" if _diff_t2 >= 0 else "red"
    _diff_icon= "▲" if _diff_t2 >= 0 else "▼"
    _diff_sub = "Surplus ▲" if _diff_t2 >= 0 else "Shortfall ▼"

    st.markdown(f"""
    <div class="cfa-kpi-grid">
        <div class="cfa-kpi blue">
            <div class="cfa-kpi-lbl">CFA FG Stock</div>
            <div class="cfa-kpi-val">{cfa_fg:,.0f}</div>
            <div class="cfa-kpi-sub">At CFA warehouses</div>
        </div>
        <div class="cfa-kpi {_stn_tile_cls}">
            <div class="cfa-kpi-lbl">STN In-Transit</div>
            <div class="cfa-kpi-val">{stn_cfa_qty:,.0f}</div>
            <div class="cfa-kpi-sub">{_stn_sub}</div>
        </div>
        <div class="cfa-kpi blue">
            <div class="cfa-kpi-lbl">{_avail_label}</div>
            <div class="cfa-kpi-val">{_total_avail_t2:,.0f}</div>
            <div class="cfa-kpi-sub">Effective available qty</div>
        </div>
        <div class="cfa-kpi amber">
            <div class="cfa-kpi-lbl">Open PO Qty</div>
            <div class="cfa-kpi-val">{open_so_qty:,.0f}</div>
            <div class="cfa-kpi-sub">Excl. Cancelled &amp; Closed</div>
        </div>
        <div class="cfa-kpi {_diff_cls}">
            <div class="cfa-kpi-lbl">Net Diff ({("FG+STN" if include_stn else "FG Only")})</div>
            <div class="cfa-kpi-val">{_diff_icon} {abs(_diff_t2):,.0f}</div>
            <div class="cfa-kpi-sub">{_diff_sub}</div>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if not df_stn.empty:
        ...
        if fg_code_col and to_wh_col and stat_col and qty_col_stn:
            ...
            if sel_cfa2 != "All CFAs":
                stn_filt = stn_filt[stn_filt[to_wh_col].astype(str) == sel_cfa2]
            if search:
    ...
    if not df_sos.empty and "SO Status" in df_sos.columns:
        ...
        if sku_col_so and wh_col_so and qty_col_so:
            ...
            if sel_cfa2 != "All CFAs":
                sos_open = sos_open[sos_open[wh_col_so].astype(str) == sel_cfa2]
            if search:
    ...
    if not stn_agg.empty:
        ...
        merged["STN In-Transit"] = 0; merged["STN Transfers"] = 0
    if not so_agg.empty:
        merged = merged.merge(so_agg, on=["Item SKU","CFA Warehouse"], how="outer")
    else:
        merged["Open PO Qty"] = 0; merged["Open Orders"] = 0
    ...
    if merged.empty:
        ...
        # KEY — Total Available switches based on mode
        merged["Total Available"] = (merged["FG Stock"] + merged["STN In-Transit"]) if include_stn else merged["FG Stock"]
        ...
        for cfa_wh in sorted(merged["CFA Warehouse"].dropna().astype(str).unique()):
            ...
            fill_rows.append({
                "CFA": cfa_wh, "Fill Rate %": round(fill_pct, 1),
        ...
        # ── Fill Rate | Expiry Heatmap layout ─────────────────────────────────
        col_fill, col_exp = st.columns([1, 1.7], gap="medium")

        with col_fill:
            st.markdown(f"""
            <div class="cfa-hdr">
                <span class="cfa-hdr-title">📊 Fill Rate by CFA</span>
                <span class="cfa-hdr-badge">Mode: {'FG+STN' if include_stn else 'FG Only'}</span>
            </div>
            """, unsafe_allow_html=True)
            ...
            if fill_df.empty:
                ...
                fill_html = '<div class="fill-panel"><div class="fill-panel-title">CFA Warehouse Fill Rates</div>'
                for _, fr in fill_df.iterrows():
                    ...
                    if pct >= 90:
                        bar_c = "#22c55e"; pct_c = "#4ade80"; chip = f'<span class="chip-ok">✔ OK</span>'
                    elif pct >= 60:
                        bar_c = "#f59e0b"; pct_c = "#fbbf24"; chip = f'<span class="chip-warn">⚠ Partial</span>'
                    else:
                        bar_c = "#ef4444"; pct_c = "#f87171"
                        chip = f'<span class="chip-crit">⚠ {short} short</span>' if short > 0 else f'<span class="chip-crit">Low</span>'
                    fill_html += f"""
                    <div class="fill-row">
                        <div class="fill-cfa-name" title="{fr['CFA']}">{fr['CFA']}</div>
                        <div><div class="fill-bar-wrap"><div class="fill-bar-inner" style="width:{min(pct,100):.1f}%;background:{bar_c};"></div></div></div>
                        <div class="fill-pct" style="color:{pct_c};">{pct:.1f}%</div>
                        <div class="fill-short">{chip}</div>
                    </div>"""
                fill_html += '</div>'
        ...
        with col_exp:
            st.markdown("""
            <div class="cfa-hdr">
                <span class="cfa-hdr-title">🔥 Expiry Heatmap by CFA</span>
                <span class="cfa-hdr-badge">Qty by Window</span>
            </div>
            """, unsafe_allow_html=True)
            ...
            if expiry_df.empty:
                ...
                bkt_cols = ["#ef4444","#f87171","#f59e0b","#fbbf24","#22c55e"]
                bkt_bgs  = [
                    "rgba(239,68,68,{a})", "rgba(248,113,113,{a})",
                    "rgba(245,158,11,{a})", "rgba(251,191,36,{a})",
                    "rgba(34,197,94,{a})"
                ]
                max_vals = {b: max(expiry_df[b].max(), 1) for b in buckets}

                hm = '<div class="hm-wrap"><div class="hm-head">'
                hm += '<div class="hm-hcell">CFA</div>'
                for b, bc in zip(buckets, bkt_cols):
                    hm += f'<div class="hm-hcell" style="color:{bc};">{b}</div>'
                hm += '</div>'

                for _, er in expiry_df.sort_values("< 30 days", ascending=False).iterrows():
                    hm += '<div class="hm-row">'
                    hm += f'<div class="hm-cell">{er["CFA"]}</div>'
                    for b, bc, bg_fmt in zip(buckets, bkt_cols, bkt_bgs):
                        val = float(er[b])
                        alpha = min(0.12 + (val / max_vals[b]) * 0.70, 0.85) if val > 0 else 0
                        cell_bg = bg_fmt.format(a=f"{alpha:.2f}") if val > 0 else "transparent"
                        val_str = f"{val:,.0f}" if val > 0 else "—"
                        txt_c   = bc if val > 0 else "#1e2535"
                        fw      = "700" if val > 0 else "400"
                        hm += f'<div class="hm-cell" style="background:{cell_bg};color:{txt_c};font-weight:{fw};">{val_str}</div>'
                    hm += '</div>'

                hm += '<div class="hm-total"><div class="hm-tcell">TOTAL</div>'
                for b, bc in zip(buckets, bkt_cols):
                    hm += f'<div class="hm-tcell" style="color:{bc};">{expiry_df[b].sum():,.0f}</div>'
                hm += '</div></div>'
                st.markdown(hm, unsafe_allow_html=True)

        st.markdown('<hr style="border:none;border-top:1px solid #161d2e;margin:16px 0;">', unsafe_allow_html=True)

        # ── Fill rate map back to merged ──────────────────────────────────────
        if not fill_df.empty:
            merged["Fill Rate %"] = merged["CFA Warehouse"].map(dict(zip(fill_df["CFA"], fill_df["Fill Rate %"]))).fillna(0.0)
        else:
        ...
        st.markdown(f"""
        <div class="cfa-hdr">
            <span class="cfa-hdr-title">📦 SKU · CFA Availability Table</span>
            <span class="cfa-hdr-badge">{len(df_disp):,} SKUs · {'FG+STN' if include_stn else 'FG Only'}</span>
        </div>
        """, unsafe_allow_html=True)
        ...
        if sel_key and batches_show:
            ...
            for _, b in batch_df.iterrows():
                ...
                if pct > 60:   bc = "#22c55e"; tc = "#4ade80"
                elif pct > 30: bc = "#f59e0b"; tc = "#fbbf24"
                else:          bc = "#ef4444"; tc = "#f87171"
                b_rows_html += f"""
                <div class="batch-row">
                    <span style="min-width:130px;color:#94a3b8;font-family:'IBM Plex Mono',monospace;font-size:11px;">{b['Batch No']}</span>
                    <div style="flex:1;background:#1e2535;border-radius:2px;height:9px;max-width:240px;">
                        <div style="width:{bar_w}%;background:{bc};height:9px;border-radius:2px;"></div>
                    </div>
                    <span style="min-width:50px;text-align:right;color:{tc};font-weight:700;font-family:'IBM Plex Mono',monospace;font-size:11px;">{pct:.1f}%</span>
                    <span style="min-width:88px;text-align:right;color:#64748b;font-family:'IBM Plex Mono',monospace;font-size:11px;">{b['Qty']:,.0f} units</span>
                    <span style="min-width:98px;color:#475569;font-size:11px;">Exp: {b['Expiry Date']}</span>
                </div>"""
            wh_lbl  = sel_key[2] if len(sel_key) > 2 else ""
            sku_lbl = sel_key[0]
            st.markdown(f"""
            <div class="batch-panel">
                <div class="batch-panel-title">
                    📋 Batch Shelf Life — {wh_lbl}
                    <span style="font-family:'IBM Plex Mono',monospace;color:#818cf8;margin-left:8px;">{sku_lbl}</span>
                    <span style="background:#0a1628;border:1px solid #1e3a5f;border-radius:3px;padding:2px 8px;font-size:10px;color:#60a5fa;margin-left:8px;">{len(batch_df)} batches</span>
                </div>
                {b_rows_html}
            </div>
            """, unsafe_allow_html=True)
        else:
            st.markdown('<div class="hint-box">↕ Select a row in the table below to view batch-level shelf life &amp; expiry breakdown</div>', unsafe_allow_html=True)
        ...
        event = st.dataframe(
            df_disp.style.apply(colour_row_cfa, axis=1),
            use_container_width=True, height=480, hide_index=False,
            column_config=col_cfg, on_select="rerun", selection_mode="single-row",
            key="cfa_table"
        )
        ...
        st.markdown(f"""
        <div class="cfa-hdr" style="margin-top:18px;">
            <span class="cfa-hdr-title">🏭 Breakdown by CFA Warehouse</span>
            <span class="cfa-hdr-badge">Click to expand</span>
        </div>
        """, unsafe_allow_html=True)
        for cfa_wh in sorted(df_disp["CFA Warehouse"].dropna().astype(str).unique()):
            ...
            with st.expander(label, expanded=False):
                st.dataframe(
                    cfa_data.style.apply(colour_row_cfa, axis=1),
                    use_container_width=True,
                    height=min(60 + len(cfa_data) * 36, 420),
                    hide_index=True, column_config=col_cfg
                )

        # ── Telegram — sends Excel file as document ───────────────────────────
        st.markdown("""
        <div class="cfa-hdr" style="margin-top:18px;">
            <span class="cfa-hdr-title">📬 Send CFA Report via Telegram</span>
            <span class="cfa-hdr-badge">Sends as Excel file</span>
        </div>
        """, unsafe_allow_html=True)
        ...
        if not _tok or not _cid:
            st.warning("⚠️ Enter Bot Token and Chat ID in the sidebar to enable Telegram.")
        else:
            tg_col1, tg_col2 = st.columns([2, 1])
            with tg_col1:
                st.markdown("""
                <div style="background:#060d18;border:1px solid #1e2535;border-radius:10px;padding:10px 16px;font-size:11px;color:#475569;font-family:'JetBrains Mono',monospace;">
                    📎 Report will be sent as an <b style="color:#93c5fd;">Excel file (.xlsx)</b> with two sheets:
                    <b style="color:#4ade80;">CFA Analysis</b> (full table) + <b style="color:#fbbf24;">Shortfall SKUs</b> (Diff &lt; 0 only)
                </div>
                """, unsafe_allow_html=True)
            with tg_col2:
                if st.button("📬  Send Excel to Telegram", use_container_width=True, key="tg_cfa_btn"):
                    try:
                        import requests as _rq, tempfile, os

                        # Build Excel with two sheets
                        _buf_tg = io.BytesIO()
                        with pd.ExcelWriter(_buf_tg, engine="openpyxl") as _wx:
                            df_export.to_excel(_wx, index=False, sheet_name="CFA Analysis")
                            _shortfall_df = df_export[df_export["Diff"] < 0].copy()
                        ...
                        _caption = (
                            f"📦 <b>YogaBar · CFA FG Report</b>\n"
                            f"🕐 {_now}\n"
                            f"📐 Mode: <b>{stn_mode}</b>\n"
                            f"{'⚠️' if _n_short > 0 else '✅'} <b>{_n_short} shortfall SKU(s)</b>\n"
                            f"📎 Full CFA Analysis + Shortfall SKUs (2 sheets)"
                        )

                        _fname = f"CFA_Report_{datetime.today().strftime('%d%b%Y')}.xlsx"
                        _url   = f"https://api.telegram.org/bot{_tok}/sendDocument"
                        _resp  = _rq.post(
                            _url,
                            data={"chat_id": _cid, "caption": _caption, "parse_mode": "HTML"},
                            files={"document": (_fname, _buf_tg.getvalue(),
                                   "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")},
                            timeout=30
                        )
                        if _resp.status_code == 200:
                            st.success(f"✅ Excel report sent to Telegram! ({_n_short} shortfall SKUs flagged)")
                        else:
                            st.error(f"❌ Failed: {_resp.json().get('description', _resp.text)}")
                    except Exception as _e:
                        st.error(f"❌ Error sending Telegram: {_e}")
    ...
    if df_mapper.empty:
        ...
        if not all_channels:
            ...
            def render_channel_tab(channel, ch_sos):
                ...
                if stn_disp.empty:
                    ...
                    with _stn_h2:
                        ...
                        st.download_button(f"⬇ STN Possible ({n_possible})", _buf_stn_possible.getvalue(), f"STN_Possible_{channel}.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True, key=f"stn_possible_dl_{channel}")

                    with _stn_h3:
                        st.markdown("<div style='margin-top:6px'></div>", unsafe_allow_html=True)
                        _tg_tok = st.session_state.get("tg_token",""); _tg_cid = st.session_state.get("tg_chat_id","")
                        if st.button(f"📬 Telegram STN ({n_possible})", use_container_width=True, disabled=(not _tg_tok or not _tg_cid or n_possible == 0), key=f"tg_stn_btn_{channel}"):
                ...
                with full_dl_placeholder: st.download_button("⬇ Full Export", buf_ch.getvalue(), f"Channel_{channel}_Full.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True, key=f"full_dl_{channel}")
                with short_dl_placeholder: st.download_button("⬇ Shortfall", buf_short.getvalue(), f"Shortfall_{channel}.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True, key=f"short_dl_{channel}")
            ...
            with ch_tabs[0]:
                ...
                all_sos["_channel_label"] = all_sos["_channel"]
