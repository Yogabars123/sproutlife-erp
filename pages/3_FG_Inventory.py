...
with k1:
    st.markdown(f"""<div class="kpi-card" style="background:linear-gradient(135deg,#1A56DB,#2563EB);">
        <div class="kpi-title">Available Quantity</div>
        <div class="kpi-value">{total_qty:,.0f}</div>
        <div class="kpi-sub">{len(df):,} filtered records</div></div>""", unsafe_allow_html=True)
with k2:
    st.markdown(f"""<div class="kpi-card" style="background:linear-gradient(135deg,#F59E0B,#FBBF24);">
        <div class="kpi-title">Expiring in 30 Days</div>
        <div class="kpi-value">{expiring_soon:,}</div>
        <div class="kpi-sub">Requires attention</div></div>""", unsafe_allow_html=True)
with k3:
    st.markdown(f"""<div class="kpi-card" style="background:linear-gradient(135deg,#DC2626,#EF4444);">
        <div class="kpi-title">Expired Batches</div>
        <div class="kpi-value">{expired_count:,}</div>
        <div class="kpi-sub">Past expiry date</div></div>""", unsafe_allow_html=True)
